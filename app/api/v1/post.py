...
@router.get("/get/{id}")
async def get_post(id: int, db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
...
@router.post("/create")
async def create_post(data: PostCreate, db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
...
async def update_post(id: int, data: PostUpdate, db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
    post = (await db.execute(select(SysPost).where(SysPost.id == id, SysPost.deleted == 0))).scalar_one_or_none()
    if not post:
        raise HTTPException(status_code=404, detail="岗位不存在")
...
async def delete_post(id: int, db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
    post = (await db.execute(select(SysPost).where(SysPost.id == id, SysPost.deleted == 0))).scalar_one_or_none()
    if not post:
        raise HTTPException(status_code=404, detail="岗位不存在")
    post.deleted = 1
